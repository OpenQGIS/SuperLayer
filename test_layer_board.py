...
class TestLayerBoardWidget(unittest.TestCase):
    ...
    def test_widget_init(self):
        ...
        self.assertEqual(len(widget.right_tab_widget.tabs), 3)
        ...
        self.assertEqual(len(widget.tab_widget.tabs), 3)
        self.assertEqual(widget.tab_widget.tabs[0][1], "矢量图层")
        self.assertEqual(widget.tab_widget.tabs[1][1], "栅格图层")
        self.assertEqual(widget.tab_widget.tabs[2][1], "操作日志")
    ...
    def test_populate_tables(self, mock_project_inst):
        ...
        self.assertEqual(widget.vector_table.item(1, 2)._flags, Qt.ItemIsSelectable | Qt.ItemIsEnabled)
    ...
    def test_apply_style(self):
        ...
        # 2. With styleWidget having apply() and styleLayer
        mock_style_widget = MagicMock(spec=['apply'])
        ...
        # 3. With styleWidget having renderer() and styleLayer
        mock_renderer_widget = MagicMock(spec=['renderer'])
        mock_renderer = MagicMock()
        mock_renderer_widget.renderer.return_value = mock_renderer
        mock_layer2 = MagicMock()
        
        widget.styleWidget = mock_renderer_widget
        widget.styleLayer = mock_layer2
        
        widget.applyStyle()
        mock_renderer_widget.renderer.assert_called_once()
        mock_layer2.setRenderer.assert_called_once()
    ...
    @patch('layer_board_widget.QgsProject.instance')
    @patch('layer_board_widget.QMenu')
    def test_context_menu_on_layer_board(self, mock_qmenu_cls, mock_project_inst):
        mock_vlayer = MagicMock()
        mock_vlayer.id.return_value = "v1"
        mock_vlayer.name.return_value = "VectorLayer"
        mock_vlayer.isSpatial.return_value = True
        ...
        mock_dock = MagicMock()
        widget = LayerBoardWidget(self.iface, mock_dock)
        
        # Manually populate row 0 column 0 of the table widget
        from layer_board_widget import QTableWidgetItem
        item_0 = QTableWidgetItem("v1")
        item_0.setData(Qt.ItemDataRole.EditRole, "v1")
        widget.vector_table.setItem(0, 0, item_0)
        
        # 1. Get selected layers when nothing is selected
        self.assertEqual(widget.getSelectedLayers(widget.vector_table), [])
        ...
        # 2. Get selected layers when row 0 is selected
        sel = widget.getSelectedLayers(widget.vector_table)
        self.assertEqual(sel, [mock_vlayer])
        
        # 3. Test context menu popup
        mock_menu = MagicMock()
        mock_qmenu_cls.return_value = mock_menu
        
        # Setup mock actions
        mock_act_change_ds = MagicMock()
        mock_menu.addAction.side_effect = lambda text: mock_act_change_ds if "更换数据源" in text else MagicMock()
        mock_menu.exec.return_value = mock_act_change_ds
        mock_menu.exec_.return_value = mock_act_change_ds
        
        widget.showTableContextMenu(widget.vector_table, None)
        
        # Verify action triggers the mock dock widget method
        mock_dock.action_change_datasource.assert_called_once_with(mock_vlayer)

    @patch('layer_board_widget.QgsProject.instance')
    @patch('layer_model.is_layer_effectively_visible')
    def test_filter_visible_only(self, mock_is_eff_visible, mock_project_inst):
        # Mock vector layer 1 (visible)
        mock_vlayer1 = MagicMock()
        mock_vlayer1.type.return_value = 0 # Vector
        mock_vlayer1.id.return_value = "v1"
        mock_vlayer1.name.return_value = "VisibleVector"
        mock_vlayer1.isSpatial.return_value = True
        mock_vlayer1.dataProvider().name.return_value = "ogr"
        mock_vlayer1.dataProvider().dataSourceUri.return_value = "v_uri1"
        mock_vlayer1.listStylesInDatabase.return_value = (0, [], [], [], [])
        
        # Mock vector layer 2 (hidden inside invisible group)
        mock_vlayer2 = MagicMock()
        mock_vlayer2.type.return_value = 0 # Vector
        mock_vlayer2.id.return_value = "v2"
        mock_vlayer2.name.return_value = "HiddenVector"
        mock_vlayer2.isSpatial.return_value = True
        mock_vlayer2.dataProvider().name.return_value = "ogr"
        mock_vlayer2.dataProvider().dataSourceUri.return_value = "v_uri2"
        mock_vlayer2.listStylesInDatabase.return_value = (0, [], [], [], [])
        ...
        mock_project_inst.return_value = mock_project
        
        widget = LayerBoardWidget(self.iface)
        
        # Side effect: v1 effectively visible, v2 not (group invisible)
        def is_eff_visible(layer):
            return layer.id() == "v1"
        mock_is_eff_visible.side_effect = is_eff_visible
        
        # 1. Without filtering: both should be in table
        widget.filter_visible_only = False
        widget.populateLayerTable('vector')
        self.assertEqual(widget.vector_table.rows, 2)
        
        # 2. With filtering: only v1 should be in table (v2's group is invisible)
        widget.filter_visible_only = True
        widget.populateLayerTable('vector')
        self.assertEqual(widget.vector_table.rows, 1)
